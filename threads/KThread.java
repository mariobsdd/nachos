...
public class KThread {
    ...
    public static void finish() {
        ...
        if (currentThread.jQueue != null){
            KThread waitingThread = currentThread.jQueue.nextThread();
            while (waitingThread != null){
                waitingThread.ready();
                waitingThread = currentThread.jQueue.nextThread();
            }
        }
...
private static class SpeakerTest implements Runnable{
    ...
    public void run(){
        for (int i=0; i<2; i++) {
            int random = 2000 + (int)(Math.random() * 5000);
            this.com.speak(random);
            System.out.println(currentThread.getName()+ " se va a dormir: "+random);
...
public static void selfTest() {
    ...
    */
    ...
    Lib.debug(dbgThread, "Enter KThread.selfTest"); 
    cero = new KThread(new ListenerTest(com)).setName("daddyshome");
    cero.fork();
    uno = new KThread(new SpeakerTest(com)).setName("margarito");
    uno.fork();
    dos = new KThread(new SpeakerTest(com)).setName("paco");
    dos.fork();
    tres = new KThread(new ListenerTest(com)).setName("fuckencio");
    tres.fork(); 
    cero.join();
    uno.join();
    dos.join();
